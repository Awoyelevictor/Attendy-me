...
export default function WorkerDashboard() {
  ...
  const [isDemoStreak, setIsDemoStreak] = useState(false);
    ...
    <div className="space-y-6 md:space-y-8 pb-12 relative">
      ...
      <AnimatePresence>
        {showSuccessOverlay && (
          ...
          >
            {/* Particle Burst */}
            {(streak > 0 || isDemoStreak || checkInRewardData?.streakIncreased) && (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs md:text-sm text-slate-400 mt-1 flex items-center gap-2 flex-wrap">
                    ...
                    <button
                      onClick={() => {
                        setIsDemoStreak(true);
                        setShowSuccessOverlay(true);
                        playFireSound();
                        setTimeout(() => {
                          setShowSuccessOverlay(false);
                          setIsDemoStreak(false);
                        }, 4000);
                      }}
                      className="flex items-center gap-1 text-orange-400 hover:text-orange-300 font-bold bg-orange-500/10 hover:bg-orange-500/20 px-2.5 py-0.5 rounded-full text-xs transition-all cursor-pointer border border-orange-500/20"
                      title="Click to test the Fire Celebration Animation!"
                    >
                      <Flame size={12} className="text-orange-500 animate-bounce" />
                      {streak > 0 ? `${streak}-Day Streak 🔥` : 'Test Fire Animation 🔥'}
                    </button>
                  </p>
                  ...
                  {!isOutOfGeofence && userCoords && !data?.checkedInToday && (
                    <motion.div
                      initial={{ opacity: 0, scale: 0.95 }}
                      animate={{ opacity: 1, scale: 1 }}
