...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await dbStore.setResetPasswordOtp(email, otp, expiresAt, resetToken);
    ...
    res.json({
      ...
      email: user.email,
      expiresAt,
      // Provide sandbox helper for instantaneous preview if running without SMTP server
      sandboxOtp: mailResult.simulated ? otp : undefined,
    });
  } catch (error) {
...
router.post('/verify-otp', async (req, res) => {
  ...
  try {
    const user = await dbStore.verifyResetPasswordOtp(email, otp.trim());
